# Engineering Evolution Prompt — Day 08

> Capability: **Finance Data Tooling**

## 今日目标

把东方财富/同花顺等数据能力封装成受控 Tool，并加入限流/缓存/超时。

## 本日工程边界

- **Before**：Day 7 Agent Runtime 已经稳定。
- **After**：Agent 可以安全访问行情/基本面数据，外部依赖具备工程保护。
- **建议触点**：`finance tool + adapter + cache/rate limit`
- **核心概念**：Tool、Adapter、RateLimiter、Cache、Timeout、Fallback
- **Java Mental Model**：DAO/SDK Adapter + Resilience4j style protection
...
本次任务必须在现有 Day 1~7 工程能力基础上做**增量演进**，而不是重建第二套 Runtime。
...
5. 当前 `.specs/` 中 Day 1~Day 7 的 `spec.md/plan.md/tasks.md/checklist.md`
6. 当前已有模块、接口、State、Graph、Tool、LLM Gateway、Event 和持久化边界

在动代码前先输出“当前能力地图”：已有能力、可复用点、Day 08 插入点。
...
**Before**：Day 1~Day 7 当前链路。

**Change**：今天新增 `Finance Data Tooling`，说明新增 Node / Service / Adapter / Contract 中哪些最小元素。
...
`/lianghua/agent/.specs/day08_*/spec.md`
`/lianghua/agent/.specs/day08_*/plan.md`
`/lianghua/agent/.specs/day08_*/tasks.md`
`/lianghua/agent/.specs/day08_*/checklist.md`
...
Day 08 完成后，必须至少回归检查 Day 1、Day 3、Day 4、Day 7 的核心能力；并根据前置依赖追加本阶段关键回归。
...
只有 checklist 中的 DoD 全部满足，才能说“Day 08 完成”。
## 8. Day 08 特殊验收
