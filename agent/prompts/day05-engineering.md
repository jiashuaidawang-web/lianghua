# Engineering Evolution Prompt — Day 05

> Capability: **Planner Capability**

## 今日目标

让 Agent 根据自然语言任务动态生成标准化 Task Plan 并写入 State。

## 本日工程边界

- **Before**：Day 4 已有图和 State，但任务流是固定的。
- **After**：Agent 可以动态规划 7 类标准原子任务，并将计划纳入后续执行。
- **建议触点**：`planner node + task model + state reducer`
- **核心概念**：PlannerNode、Task、Structured Output、State mutation/reducer
- **Java Mental Model**：流程定义解析器 + DAG/工作流编排
...
本次任务必须在现有 Day 1~4 工程能力基础上做**增量演进**，而不是重建第二套 Runtime。
...
5. 当前 `.specs/` 中 Day 1~Day 4 的 `spec.md/plan.md/tasks.md/checklist.md`
6. 当前已有模块、接口、State、Graph、Tool、LLM Gateway、Event 和持久化边界

在动代码前先输出“当前能力地图”：已有能力、可复用点、Day 05 插入点。
...
**Before**：Day 1~Day 4 当前链路。

**Change**：今天新增 `Planner Capability`，说明新增 Node / Service / Adapter / Contract 中哪些最小元素。
...
`/lianghua/agent/.specs/day05_*/spec.md`
`/lianghua/agent/.specs/day05_*/plan.md`
`/lianghua/agent/.specs/day05_*/tasks.md`
`/lianghua/agent/.specs/day05_*/checklist.md`
...
Day 05 完成后，必须至少回归检查 Day 1、Day 3、Day 4、Day 7 的核心能力；并根据前置依赖追加本阶段关键回归。
...
只有 checklist 中的 DoD 全部满足，才能说“Day 05 完成”。
## 8. Day 05 特殊验收

Planner 输出固定的 7 类标准 Task；Task 必须可序列化、可追踪、可被后续 Node 消费。
