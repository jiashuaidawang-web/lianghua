# Engineering Evolution Prompt — Day 09

> Capability: **MCP Capability**

## 今日目标

把已有金融工具/因子能力标准化暴露为 MCP Server，避免 Agent 与工具实现强耦合。

## 本日工程边界

- **Before**：Day 8 Tool 只通过本地 Tool registry 调用。
- **After**：同一工具能力可通过 MCP 标准协议被外部 Agent/Host 发现和调用。
- **建议触点**：`mcp server + tool bridge + transport`
- **核心概念**：MCP Server、Tool、Resource、Prompt、schema/metadata
- **Java Mental Model**：gRPC/OpenRPC service contract
...
本次任务必须在现有 Day 1~8 工程能力基础上做**增量演进**，而不是重建第二套 Runtime。
...
5. 当前 `.specs/` 中 Day 1~Day 8 的 `spec.md/plan.md/tasks.md/checklist.md`
6. 当前已有模块、接口、State、Graph、Tool、LLM Gateway、Event 和持久化边界

在动代码前先输出“当前能力地图”：已有能力、可复用点、Day 09 插入点。
...
**Before**：Day 1~Day 8 当前链路。

**Change**：今天新增 `MCP Capability`，说明新增 Node / Service / Adapter / Contract 中哪些最小元素。
...
`/lianghua/agent/.specs/day09_*/spec.md`
`/lianghua/agent/.specs/day09_*/plan.md`
`/lianghua/agent/.specs/day09_*/tasks.md`
`/lianghua/agent/.specs/day09_*/checklist.md`
...
Day 09 完成后，必须至少回归检查 Day 1、Day 3、Day 4、Day 7 的核心能力；并根据前置依赖追加本阶段关键回归。
...
只有 checklist 中的 DoD 全部满足，才能说“Day 09 完成”。
## 8. Day 09 特殊验收

MCP 只暴露标准能力，不复制一套本地 Tool 逻辑。已有 Tool 应通过 bridge/adapter 被 MCP 复用。
