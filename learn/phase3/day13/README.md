# Day 13: Socratic Action Suggestion 协议

> Phase 3: Socratic 引导与 HITL 挂起恢复
...
```mermaid
graph TD
    Agent[Agent] --> Diagnose[Socratic Diagnose]
    Diagnose --> Suggest[Socratic Action JSON]
    Suggest --> Gate{Human / Risk Gate}
    Gate -->|confirm| Continue[Continue Graph]
    Gate -->|reject| Revise[Revise Plan]
    Continue --> Research[Research / RAG]
...
**核心类比：** HATEOAS → Action Suggestion；DTO Contract → 前端可渲染卡片。
...
**断点类：** `com.quant.agent.interfaces.http.ActionSuggestionController`

**观察：** `reason、evidence、suggested_actions、requires_confirmation`
...
在 /lianghua/agent/.specs/day13_socratic_action_suggestion_协议/ 创建/更新：
spec.md
plan.md
tasks.md
checklist.md

主题：Day 13 / Socratic Action Suggestion 协议
...
```text
严格按照刚才的 spec/plan/tasks 在 /lianghua/agent 中实现 Day 13。
...
完整导师 Prompt：[`/lianghua/learn/prompts/day13-learning.md`](../../../prompts/day13-learning.md)
...
进入 [`/lianghua/agent/prompts/day13-engineering.md`](../../../agent/prompts/day13-engineering.md)，让 Claude/Cursor 在现有工程上做纵向增量，不创建第二套 Demo。
