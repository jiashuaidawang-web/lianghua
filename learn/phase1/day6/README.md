# Day 6: Conditional Edges 与 Re-planning Loop

> Phase 1: LangGraph4j 状态机图编排
...
```mermaid
graph TD
    Input[User Request] --> Planner[PlannerNode]
    Planner --> State[(QuantAgentState)]
    State --> Executor[ExecutorNode]
    Executor --> Review[ReviewNode]
    Review -->|pass| End[END]
    Review -->|fail| Replan[Re-plan]
    Replan --> Planner
    State --> Saver[Checkpoint/Saver]
    Saver --> Resume[Resume by thread_id]
```
...
**核心类比：** while/if + Saga 补偿 → Conditional Edge + Re-plan Loop。
...
**断点类：** `com.quant.agent.agent.graph.RoutingNode`

**观察：** `reviewResult、routeKey、replanCount`
...
在 /lianghua/agent/.specs/day06_conditional_edges_与_re-planning_loop/ 创建/更新：
spec.md
plan.md
tasks.md
checklist.md

主题：Day 6 / Conditional Edges 与 Re-planning Loop
...
```text
严格按照刚才的 spec/plan/tasks 在 /lianghua/agent 中实现 Day 6。
...
完整导师 Prompt：[`/lianghua/learn/prompts/day06-learning.md`](../../../prompts/day06-learning.md)
...
进入 [`/lianghua/agent/prompts/day06-engineering.md`](../../../agent/prompts/day06-engineering.md)，让 Claude/Cursor 在现有工程上做纵向增量，不创建第二套 Demo。
